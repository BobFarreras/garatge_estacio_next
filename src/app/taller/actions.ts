...
import Airtable, { FieldSet } from 'airtable'; // ✅ CORRECCIÓ: Importem FieldSet
import { addDays, isBefore, startOfDay } from 'date-fns'; // ✅ CORRECCIÓ: Importem les funcions de date-fns
...
import { APPOINTMENT_CONFIG, AIRTABLE_TABLES, CLOUDINARY_FOLDERS } from '@/config/taller'; // ✅ CORRECCIÓ: Importem la configuració
...
// --- ACCIÓ PER CREAR CITA (VERSIÓ FINAL CORREGIDA) ---
export async function createAppointmentAction(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  // ✅ CORRECCIÓ: Creem un objecte a part per validar, per no tenir problemes de tipus.
  const objectToValidate = {
    ...Object.fromEntries(formData),
    privacyPolicy: formData.get('privacyPolicy') === 'on',
  };
  console.log("🟢 SERVER: Dades rebudes per validar:", objectToValidate);
  ...
  // 2. Validacions de Lògica de Negoci al Servidor (Capa de Seguretat)
  ...
  let airtableRecordId: string | null = null; // ✅ 1. Declarem l'ID aquí fora per a poder-lo fer servir al 'catch'

  try {
    console.log("🟢 SERVER: Iniciant comprovacions amb serveis externs (Airtable...).");
    ...
    const existingAppointments = await appointmentsTable.select({
      ...
      maxRecords: 1, // Optimització: només necessitem saber si n'hi ha almenys una
    }).firstPage();

    if (existingAppointments.length > 0) {
      console.warn("🟠 SERVER: Error de lògica - L'hora ja estava ocupada.");
      return { success: false, error: `L'hora seleccionada (${validatedData.time}) ja no està disponible.` };
    }

    // 4. Pujada d'arxius a Cloudinary
    ...
    console.log("🟢 SERVER: La cita s'ha creat correctament a Airtable i Google Calendar.");

    // 6. Integracions (Google Calendar & Emails)
