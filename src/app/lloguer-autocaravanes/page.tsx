...
import { Loader2, User, Mail, Phone, Calendar as CalendarIcon, Users, ArrowRight, BedDouble, Droplets, Sun, Info } from 'lucide-react';
...
// ✅ Importem els tipus des del nostre fitxer central
import type { Motorhome, Pricing } from '@/types';
...
const LloguerAutocaravanes = () => {
    ...
    const [motorhomes, setMotorhomes] = useState<Motorhome[]>([]); // Ara utilitza el tipus importat
            ...
            <div className="bg-white">
                <section className="relative py-24 bg-gray-800 text-white">
                    <div className="absolute inset-0">
                        {motorhomes.length > 0 && <Image src={motorhomes[0].image_url} alt="Motorhome background" fill className="object-cover" />}
                    </div>
                ...
                <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>{t('motorhomeRentalPage.bookingDialogDescription', { motorhomeName: selectedMotorhome?.name, startDate: selectedRange?.from ? format(selectedRange.from, 'dd/MM/yyyy') : '', endDate: selectedRange?.to ? format(selectedRange.to, 'dd/MM/yyyy') : '' })}</DialogDescription>
                        </DialogHeader>
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-4">
                            <div className="grid gap-2"><Label htmlFor="customer_name">{t('motorhomeRentalPage.formNameLabel')}</Label><div className="relative"><User className="absolute right-2.5 top-2.5 h-4 w-4 text-muted-foreground" /><Input id="customer_name" {...register("customer_name")} /></div>{errors.customer_name && <p className="text-red-500 text-sm">{errors.customer_name.message}</p>}</div>
                            <div className="grid gap-2"><Label htmlFor="customer_email">{t('motorhomeRentalPage.formEmailLabel')}</Label><div className="relative"><Mail className="absolute right-2.5 top-2.5 h-4 w-4 text-muted-foreground" /><Input id="customer_email" type="email" {...register("customer_email")} /></div>{errors.customer_email && <p className="text-red-500 text-sm">{errors.customer_email.message}</p>}</div>
                            <div className="grid gap-2"><Label htmlFor="customer_phone">{t('motorhomeRentalPage.formPhoneLabel')}</Label><div className="relative"><Phone className="absolute right-2.5 top-2.5 h-4 w-4 text-muted-foreground" /><Input id="customer_phone" type="tel" {...register("customer_phone")} /></div>{errors.customer_phone && <p className="text-red-500 text-sm">{errors.customer_phone.message}</p>}</div>
                            <div className="flex items-center space-x-2">
                                <Controller name="privacyPolicy" control={control} render={({ field }) => (<Checkbox id="privacyPolicy" checked={field.value} onCheckedChange={field.onChange} />)} />
                                <label htmlFor="privacyPolicy" className="text-sm">{t('form.privacyAccept')} <a href="/politica-privacitat.html" target="_blank" rel="noopener noreferrer" className="underline text-red-600 hover:text-red-800">{t('form.privacyPolicy')}</a>.</label>
                            </div>
                            {errors.privacyPolicy && <p className="text-red-500 text-sm">{errors.privacyPolicy.message}</p>}
                            <Button type="submit" disabled={isSubmitting} className="w-full bg-red-600 hover:bg-red-700">{isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>{t('motorhomeRentalPage.submittingButton')}</> : t('motorhomeRentalPage.submitButton')}</Button>
                        </form>
                    </DialogContent>
                </Dialog>
            </div>
